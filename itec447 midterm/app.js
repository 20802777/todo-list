...
const markComplete = (text) => {
  ...
  const task = todos.find(todo => todo.text === text);
...
const deleteTask = (text) => {
  ...
  const gridCard = (tag, text, time, status) => {
    const color = tagsColor(tag);
    let icon = '';

    if(status === 'Incomplete'){
      icon = `<i class="fa fa-check-circle" onClick="markComplete('${text}')" aria-hidden="true"></i>`;
    }
    return `<div class="row__card ${color}">
      <p class="icon__area">
        <i class="fa fa-minus-circle" onclick="deleteTask('${text}')" aria-hidden="true"></i>
        ${icon}
      </p>
      <div class="row__tag">
        ${tagsIcon(tag)}
  ...
  const colCard = (tag, text, time, status) => {
    ...
    todos = [
      {
        tag: data.tag,
        text: data.name,
        time: new Date(data.date).toDateString(),
        status: "Incomplete",
      },
      ...todos,
    ];
  ...
  const renderList = (todos) => {
    ...
    todos.forEach(todo => {
      ROW__ITEMS.innerHTML += gridCard(todo.tag, todo.text, todo.time, todo.status);
    });
  };
  ...
  function filterTasks() {
    ...
    if(filterValue === 'completed'){
      filteredList = todos.filter(todo => todo.status === 'Complete');
    }else if(filterValue ==='uncompleted'){
      filteredList = todos.filter(todo => todo.status === 'Incomplete');
    }else{
      filteredList = todos;
    }
